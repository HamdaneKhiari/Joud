/**
 * Migration 003 : labels des sous-familles pour les familles à structure prédéfinie :
 ...
 *
 * Non inclus ici (viennent du script Python via Excel) : phrase types,
 * connector et word games (pas de sous-familles, subfamily_id=0).
 */
...
export default createMigration(
  ...
  async (db: SQLite.SQLiteDatabase) => {
    ...
    for (const [familySlug, subfamilies] of Object.entries(VOCAB_SUBFAMILY_CONFIG)) {
      ...
      if (!family) {
        console.warn(`[Migration 003] Family '${familySlug}' not found, skipping`);
        continue;
      }
    ...
    for (const [famSlug, subId, identity, title, icon, desc] of DIALOGUES_SUBFAMILY_SEEDS) {
      ...
      if (!family) {
        console.warn(`[Migration 003] Dialogue family '${famSlug}' not found, skipping`);
        continue;
      }
    ...
    for (const [famSlug, subId, identity, title, icon, desc] of READING_SUBFAMILY_SEEDS) {
      ...
      if (!family) {
        console.warn(`[Migration 003] Reading family '${famSlug}' not found, skipping`);
        continue;
      }
    ...
    console.log('[Migration 003] ✓ Subfamily labels seedés (vocab, dialogues, reading)');
  }
);
